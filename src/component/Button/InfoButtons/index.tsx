...
export const InfoButtons: React.FC<{ selected: string }> = (props) => {
  ...
  const [hotelSelected] = useState(props.selected === 'hotel');
  const [transportSelected] = useState(props.selected === 'transport');
  const [covidSelected] = useState(props.selected === 'covid');
